...
fn push<F>(fns: &mut Vec<NestedValidationFn<Box<HashMap<MapKey, Value>>>>, name: Arc<String>, f: Arc<F>)
where
...
pub(crate) fn make_validate_map(m: &mut HashMap<String, ValidationFn>, field: FieldDescriptor, rules: &FieldRules) -> Vec<NestedValidationFn<Box<HashMap<MapKey, Value>>>> {
    let mut fns = Vec::new();
    ...
    if let Some(Type::Map(rules)) = &rules.r#type {
        ...
        if rules.min_pairs.is_some() {
            push(&mut fns, name.clone(), Arc::new(move |vals: &HashMap<MapKey, Value>, rules: &MapRules, name: &String, _: &HashMap<String, ValidationFn>| {
        ...
        if let Some(ref rules) = rules.keys {
            ...
            push(&mut fns, name.clone(), Arc::new(move |vals: &HashMap<MapKey, Value>, rules: &MapRules, _: &String, m: &HashMap<String, ValidationFn>| {
        ...
        if rules.no_sparse.unwrap_or(false) {
            ...
            push(&mut fns, name.clone(), Arc::new(move |vals: &HashMap<MapKey, Value>, _: &MapRules, name: &String, _: &HashMap<String, ValidationFn>| {
                let kind = kind.clone();
                for (_, val) in vals {
