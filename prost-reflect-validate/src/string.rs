...
pub(crate) fn validate_string(val: Option<&str>, field: &FieldDescriptor, rules: &FieldRules) -> anyhow::Result<()> {
    ...
    if let Some(m) = rules.min_bytes {
        if bytes.len() < m as usize {
            return Err(format_err!("{}: must be minimum {} bytes long", field.full_name(), m));
        }
    }
    if let Some(m) = rules.max_bytes {
        if bytes.len() > m as usize {
            return Err(format_err!("{}: must be maximum {} bytes long", field.full_name(), m));
        }
    }
    ...
    if let Some(v) = &rules.not_contains {
        if val.contains(v) {
            return Err(format_err!("{}: must not contains {}", field.full_name(), v));
        }
    }
    if !rules.r#in.is_empty() && !rules.r#in.contains(&val.to_string()) {
        return Err(format_err!("{}: must be in {:?}", field.full_name(), rules.r#in));
    }
    if !rules.not_in.is_empty() && !rules.not_in.contains(&val.to_string()) {
    ...
    match rules.well_known.unwrap() {
        WellKnown::Email(v) => {
            if v && !validate_email(val) {
                return Err(format_err!("{}: must be a valid email", field.full_name()));
            }
        }
        WellKnown::Hostname(v) => {
            if v && !validate_domain_part(val) {
                return Err(format_err!("{}: must be a valid hostname", field.full_name()));
            }
        }
        WellKnown::Ip(v) => {
            if v && !val.validate_ip() {
                return Err(format_err!("{}: must be a valid ip", field.full_name()));
            }
        }
        WellKnown::Ipv4(v) => {
            if v && !val.validate_ipv4() {
                return Err(format_err!("{}: must be a valid ipv4", field.full_name()));
            }
        }
        WellKnown::Ipv6(v) => {
            if v && !val.validate_ipv6() {
                return Err(format_err!("{}: must be a valid ipv6", field.full_name()));
            }
        }
        WellKnown::Uri(v) => {
            if v && !validate_uri(val) {
                return Err(format_err!("{}: must be a valid absolute URI", field.full_name()));
            }
        }
        WellKnown::UriRef(v) => {
            if v && !validate_uri_ref(val) {
                return Err(format_err!("{}: must be a valid URI", field.full_name()));
            }
        }
        WellKnown::Address(v) => {
            if v && !validate_domain_part(val) && !val.validate_ip() {
                return Err(format_err!("{}: must be a valid hostname or ip address", field.full_name()));
            }
        }
        WellKnown::Uuid(v) => {
            if v && !UUID_RE.is_match(val) {
                return Err(format_err!("{}: must be a valid uuid", field.full_name()));
            }
        }
        WellKnown::WellKnownRegex(v) => {
            match KnownRegex::try_from(v) {
                Ok(KnownRegex::HttpHeaderName) => {
                    if !HEADER_NAME_RE.is_match(val) {
                        return Err(format_err!("{}: must match regex pattern \"^:?[0-9a-zA-Z!#$%&'*+-.^_|~`]+$\"", field.full_name()));
                    }
                }
                Ok(KnownRegex::HttpHeaderValue) => {
                    if !HEADER_VALUE_RE.is_match(val) {
                        return Err(format_err!("{}: must match regex pattern \"^[^\\x00-\\b\\n-\\x1f\\x7f]*$\"", field.full_name()));
                    }
                }
                _ => {}
            }
