    ...
    if let Some(lt) = rules.lt.map(|v| v.as_datetime()) {
        if let Some(gt) = rules.gt.map(|v| v.as_datetime()) {
            if lt > gt {
                ...
                push(&mut fns, &name, Arc::new(move |val: &OffsetDateTime, rules: &TimestampRules, name: &String| {
                    ...
                    let gt = rules.gt.unwrap().as_datetime();
                    if *val >= lt && *val <= gt {
                        return Err(format_err!("{}: must be outside range [{}, {}]", name, gt, lt));
                    }
                    Ok(true)
                }));
            }
            ...
            if lt > gte {
                ...
                push(&mut fns, &name, Arc::new(move |val: &OffsetDateTime, rules: &TimestampRules, name: &String| {
                    ...
                    let lt = rules.lt.unwrap().as_datetime();
                    if *val >= lt && *val < gte {
                        return Err(format_err!("{}: must be outside range [{}, {})", name, lt, gte));
                    }
                    Ok(true)
                }));
            }
        ...
        if rules.within.is_some() {
            push(&mut fns, &name, Arc::new(move |val: &OffsetDateTime, rules: &TimestampRules, name: &String| {
                ...
                if *val >= now || *val < now - d {
                    return Err(format_err!("{}: must be within {} from {}", name, d.to_string(), now));
                }
                Ok(true)
            }));
        } else {
        ...
        if rules.within.is_some() {
            push(&mut fns, &name, Arc::new(move |val: &OffsetDateTime, rules: &TimestampRules, name: &String| {
                ...
                let d = rules.within.unwrap().as_duration();
                if *val <= now || *val > now + d {
                    return Err(format_err!("{}: value must be less than now within {}", name, d.to_string()));
                }
                Ok(true)
            }));
        } else {
