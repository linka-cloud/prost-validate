...
pub trait Validator: Send + Sync {
    ...
    fn validate_all(&self) -> Vec<Error> {
        vec![]
    }
}
...
mod tests {
    ...
    fn test_validator() {
        ...
        assert!(!prost_validate::validate_all!(a).is_empty());
    }
    #[test]
    ...
    fn test_non_validator() {
        ...
        assert!(prost_validate::validate_all!(b).is_empty());
    }
    #[test]
    fn test_scalar() {
        let c = &42;
        assert!(prost_validate::validate!(c).is_ok());
        assert!(prost_validate::validate_all!(c).is_empty());
    }
}
