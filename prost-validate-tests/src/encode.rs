use prost_types::Any;
use prost_validate_tests_types::proto::cases::*;
use prost_validate_tests_types::proto::harness::{TestCase, TestSuite};
use prost_validate_tests_types::CASES;

pub fn encode_suite() -> Vec<u8> {
    let suite = make_suite();
    let mut buf = Vec::new();
    prost::Message::encode(&suite, &mut buf).unwrap();
    buf
}
        ...
        ($($name:ident: $typ:ident,)*) => {
            vec![
                $({
                    ...
                    let (input, failures) = CASES.get(name.as_str()).unwrap()();
                    TestCase {
                        name,
                        failures,
                        input: Some(Any::from_msg(input.as_any().downcast_ref::<$typ>().unwrap()).unwrap()),
                    }
                },)*
            ]
        }
